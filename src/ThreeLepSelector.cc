...
void ThreeLepSelector::Init(TTree* tree)
{
...
#ifdef USETREE
    std::string testTree = "testTree" + std::to_string(testTreeNum++);
    AddObject<TTree>(treeMap["tree"], testTree.c_str(), testTree.c_str());
    ...
    treeMap["tree"]->Branch("LepCos", &bLepCos);
    treeMap["tree"]->Branch("JetLep1_Cos", &bjl1Cos);
    treeMap["tree"]->Branch("JetLep2_Cos", &bjl2Cos);
    treeMap["tree"]->Branch("JetBJet_DR", &bjbdr);
    treeMap["tree"]->Branch("Lep_DR", &bldr);
    treeMap["tree"]->Branch("JetBJet_Cos", &bjbCos);
...
void ThreeLepSelector::SetBranchesNanoAOD()
{
    ...
    if (name_tmp.find("2016") != std::string::npos)
        year_ = yr2016;
    else if (name_tmp.find("2017") != std::string::npos)
        year_ = yr2017;
    else if (name_tmp.find("2018") != std::string::npos)
        year_ = yr2018;
    else
        year_ = yr2016;
    std::cout << name_tmp << year_ << " " << yr2016 << " " << yr2017 << " " << yr2018 << "\n";
...
bool ThreeLepSelector::passMVACut(std::vector<std::vector<double>> mvaCuts, int index)
{
    ...
    if (Electron_pt[index] / CHGPT(index) < 5)
        ...
        caseIndex += 0;
    else if (Electron_pt[index] / CHGPT(index) < 15 && year_ == yr2016)
        caseIndex += 1;
    ...
    if (year_ == yr2018)
        mvaValue = atanh(Electron_MVA[index]);

    if (caseIndex % 4 != 2)
        return mvaValue > mvaCuts[caseIndex / 4][caseIndex % 4];
    else
        return mvaValue > mvaInterpolate(Electron_pt[index] / CHGPT(index), mvaCuts[caseIndex / 4]);
}
...
bool ThreeLepSelector::isGoodJet(size_t index)
{
    ...
    return ((Jet_pt[index] > ptCut) && (abs(Jet_eta[index]) < etaCut) && (yearCut));
}
...
bool ThreeLepSelector::isGoodBJet(size_t index)
{
    ...
    if (year_ == yr2016)
        ...
        yearCut = yearCut && (Jet_btagDeepB[index] > 0.4941);
    else if (year_ == yr2018)
        yearCut = yearCut && (Jet_btagDeepB[index] > 0.4184);
...
bool ThreeLepSelector::passTriggerEmu(size_t index)
{
    ...
    return (abs(Electron_eInvMinusPInv[index]) < 0.01 && etaDepend && Electron_hoe[index] < 0.08);
}
...
void ThreeLepSelector::FillHistograms(Long64_t entry, std::pair<Systematic, std::string> variation)
{
...
#ifdef TRIGGER
    /// Trigger
    if (((channel_ == mm && !HLT_DoubleMu8_Mass8_PFHT300) || (channel_ == em && !HLT_Mu8_Ele8_CaloIdM_TrackIdM_Mass8_PFHT300) || (channel_ == ee && !HLT_DoubleEle8_CaloIdM_TrackIdM_Mass8_PFHT300))
    ...
    if (selection_ != MVAStudy) {
        ...
        if (goodLeptons[0].Pt() < 25)
            return;
        Fill1D("CutFlow", ++step);

        // same sign requirement
        if ((goodLeptons.size() == 2 && goodLeptons[0].Charge() * goodLeptons[1].Charge() < 0) || (goodLeptons.size() == 3 && goodLeptons[0].Charge() * goodLeptons[2].Charge() > 0))
            return;
        Fill1D("CutFlow", ++step);
    }

    if (channel_ == mult) {
        if ((goodLeptons[0].Charge() * goodLeptons[1].Charge() > 0) && (goodLeptons[0].Charge() * goodLeptons[2].Charge() > 0))
            return;
    }
    ...
    // bjet cut
    if (selection_ == MVAStudy && nBJets < 1)
        return;
    else if (selection_ != MVAStudy && nBJets < 2)
        return;
    Fill1D("CutFlow", ++step);

    // ht cut
    if (selection_ == MVAStudy && HT < 100)
        return;
    else if (selection_ != MVAStudy && HT < 300)
        return;
    Fill1D("CutFlow", ++step);

    // met cut
    if (selection_ == MVAStudy && MET < 25)
        return;
    else if (selection_ != MVAStudy && MET < 50)
        return;
    Fill1D("CutFlow", ++step);
    ...
    HistFullFill(histMap1D_, "weight", variation.first, weight, 1);
    ...
    Fill1D("nPileup", Pileup_nTrueInt);
    ...
    bLepCos = ROOT::Math::VectorUtil::CosTheta(goodLeptons[0].v, goodLeptons[1].v);
    bjl1Cos = ROOT::Math::VectorUtil::CosTheta(goodLeptons[0].v, goodJets[0].v);
    bjl2Cos = ROOT::Math::VectorUtil::CosTheta(goodLeptons[1].v, goodJets[0].v);
    bjbdr = reco::deltaR(goodJets[bjetList.at(0)].v, goodJets[goodjet1].v);
    bldr = reco::deltaR(goodLeptons[0].v, goodLeptons[1].v);
    bjbCos = ROOT::Math::VectorUtil::CosTheta(goodJets[bjetList.at(0)].v, goodJets[goodjet1].v);
...
std::vector<GoodPart>::iterator ThreeLepSelector::findJet(std::vector<GoodPart>::iterator& start, int pid)
{
    while (start != goodJets.end()) {
        if (pid == PID_BJET && start->passedBJetSel())
            return start;
        else if (pid != PID_BJET && start->passedJetSel())
            return start;
        ++start;
    }
    return goodJets.end();
