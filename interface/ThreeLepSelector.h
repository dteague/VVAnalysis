...
#include "Analysis/VVAnalysis/interface/FourTopScales.h"
  ...
  Float_t Electron_MVA[20], Electron_cutBased[20];
  ...
  TTreeReaderArray<Int_t> Electron_tightCharge = {fReader,
  ...
  TTreeReaderArray<Float_t> Electron_deltaEtaSC = {fReader,
                                                   "Electron_deltaEtaSC"};
  TTreeReaderArray<Float_t> Electron_eInvMinusPInv = {fReader,
                                                      "Electron_eInvMinusPInv"};
  TTreeReaderArray<Float_t> Electron_dr03EcalRecHitSumEt = {
      fReader, "Electron_dr03EcalRecHitSumEt"};
  TTreeReaderArray<Float_t> Electron_dr03HcalDepth1TowerSumEt = {
      fReader, "Electron_dr03HcalDepth1TowerSumEt"};
  TTreeReaderArray<Float_t> Electron_dr03TkSumPt = {fReader,
                                                    "Electron_dr03TkSumPt"};
  // TTreeReaderArray<Int_t>     Electron_vidBitmap = {fReader,
  // "Electron_vidBitmap"};
  TTreeReaderArray<Float_t> Electron_jetRelIso = {fReader,
                                                  "Electron_jetRelIso"};
  ...
  TTreeReaderArray<Float_t> Muon_pfRelIso04_all = {fReader,
                                                   "Muon_pfRelIso04_all"};
  TTreeReaderArray<Float_t> Muon_miniPFRelIso_all = {fReader,
  ...
  TTreeReaderArray<Int_t> GenPart_genPartIdxMother = {
      tmpReader, "GenPart_genPartIdxMother"};
  ...
  std::vector<GoodPart>::iterator findJet(std::vector<GoodPart>::iterator &,
                                          int);

  // std::map<std::string, TTree*> treeMap = {{"tree", nullptr}};
  TTree *workTree = nullptr;
  std::unordered_map<std::string, Float_t> treeVars;
  ...
  void
  LoadBranchesNanoAOD(Long64_t entry,
  ...
  /// ignore
  void LoadBranchesUWVV(Long64_t entry,
                        std::pair<Systematic, std::string> variation) override {
    return;
  }
